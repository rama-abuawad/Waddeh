...
LEVEL_LABELS_AR = {
    ...
    SimplificationLevel.standard: "قياسي",
    SimplificationLevel.advanced: "متقدم",
    SimplificationLevel.original: "أصلي",
}
...
def build_adaptation_strategy(level: SimplificationLevel) -> AdaptationStrategy:
    if level == SimplificationLevel.very_easy:
        return AdaptationStrategy(
            ...
            terminology_policy="استبدل المصطلحات الصعبة مؤقتاً مع حفظها في Bridge Mode.",
        )
    if level == SimplificationLevel.easy:
        return AdaptationStrategy(
            ...
            terminology_policy="أعد إدخال بعض الكلمات الأصلية عندما تساعد المتعلم على التقدم.",
        )
    ...
    if level == SimplificationLevel.advanced:
        return AdaptationStrategy(
            target_level=level,
            target_level_label=LEVEL_LABELS_AR[level],
            vocabulary_control="اقترب من مفردات النص الأصلية ولا تستبدل إلا العبارات المربكة.",
            ...
            terminology_policy="أبق المصطلحات الأصلية ما لم تسبب لبساً واضحاً.",
        )
    return AdaptationStrategy(
        target_level=level,
        target_level_label=LEVEL_LABELS_AR[level],
        ...
        explanation_control="قدّم أدوات تعلم حول النص الأصلي فقط.",
