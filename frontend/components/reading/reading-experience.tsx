          ...
          <section key={selectedTab} className="v4-mode-content" role="tabpanel">
            ...
            {standard && selectedTab === "learn" && (
              <div className="v4-learn-mode">
                ...
                {standard.cultural_meanings.length > 0 && <section><h2>{copy.expressions}</h2><div className="v4-expression-list">{standard.cultural_meanings.map((item) => { const saved = savedWords.some((word) => word.word === item.expression && word.meaning === item.intended_meaning); return <article key={item.expression}><div><strong dir="rtl">{item.expression}</strong><p>{uiLanguage === "ar" ? item.intended_meaning : item.english_meaning}</p><small>{uiLanguage === "ar" ? item.cultural_context : item.cultural_context_english}</small></div><SavedButton saved={saved} label={copy.save} savedLabel={copy.saved} onClick={() => saveWord(expressionAsWord(item), { kind: "expression", sourceReadingId: reading.id, sourceTitle: reading.title })} /></article>; })}</div></section>}
